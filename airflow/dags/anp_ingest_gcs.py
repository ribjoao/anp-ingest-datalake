...
OUTPUT_GCS = 'workflow_anp/combustiveis_auto_{{ execution_date.strftime(\'%Y-%m\') }}.parquet'

#format name after convert dataset
parquet_filename = OUTPUT_FILE_TEMPLATE.replace('.csv', '.parquet')
...
def format_to_parquet(src_file):
    if not src_file.endswith('.csv'):
        logging.error("At the moment, only read CSV files!")
        return
...
# Tasks dependencies
    download_data_task >> format_to_parquet_task >> local_to_datalake_task